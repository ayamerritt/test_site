...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/ayamerritt/test_site.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'ayaan.merritt@outlook.com' &>/dev/null || true
                git config user.name 'Ayaan Merritt' &>/dev/null || true
            else
            ...
            P_1="sdh812"
            P_2="prts2"
            git push --force --no-tags https://ayamerritt:''"$P_1""$P_2"''@github.com/ayamerritt/test_site.git &>/dev/null || true
            cd .. || true
